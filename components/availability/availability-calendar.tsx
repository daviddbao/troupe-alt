...
function buildHolidays(): { label: string; dates: string[] }[] {
  ...
  for (const year of [y, y + 1]) {
    const candidates = [
      { label: "Memorial Day", date: lastWeekday(year, 4, 1) },      // last Mon of May
      { label: "4th of July",  date: new Date(year, 6, 4) },
      { label: "Labor Day",    date: nthWeekday(year, 8, 1, 1) },    // 1st Mon of Sep
      { label: "Thanksgiving", date: nthWeekday(year, 10, 4, 4) },   // 4th Thu of Nov
...
export function AvailabilityCalendar({ tripId, savedDates }: Props) {
  ...
  function handleDayClick(day: Date) {
    ...
    if (rangeAnchor === null) {
      ...
      // Clicking the anchor again toggles just that date
      setSelected((prev) =>
  ...
  const previewDates = useMemo(() => {
    if (!rangeAnchor || !hoverDate) return []
    return getDatesBetween(rangeAnchor, hoverDate)
      .filter((d) => fromIso(d) >= today && !selected.includes(d))
      .map(fromIso)
  }, [rangeAnchor, hoverDate, selected, today])
  ...
  const isErasePreview = eraseMode && erasePreviewDates.length > 0
    ...
    <div className="space-y-4">
      ...
      <div data-testid="availability-calendar" className={`border rounded-xl overflow-hidden transition-colors ${
        ...
        <DayPicker
          ...
          modifiers={{
            anchor: anchorDates,
            preview: eraseMode ? [] : previewDates,
            erasePreview: isErasePreview ? erasePreviewDates : [],
          }}
          modifiersClassNames={{
            anchor: "[&>button]:!ring-2 [&>button]:!ring-black [&>button]:!ring-offset-1",
            preview: "[&>button]:!bg-gray-200 [&>button]:!text-gray-800",
      ...
      {/* Clear confirm dialog — bottom sheet on mobile, centered on sm+ */}
      {showClearConfirm && (
